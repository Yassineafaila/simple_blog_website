        ...
        <form action="/posts" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="description"
                    value="{{ old('description') }}" placeholder="a small description for the post" />
                @error('description')
                    <p class="text-red-200 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select id="categories" name="categories[]" class="border border-gray-200 rounded p-2 w-full"
                    multiple="multiple">
                    @php
                        $categoryOptions = [
                    ...
                    @foreach ($categoryOptions as $key => $category)
                        <option value="{{ $key }}" {{ in_array($key, old('categories', [])) ? 'selected' : '' }}>
                            {{ $category }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <textarea class="border border-gray-200 rounded p-2 w-full" name="content" value="{{ old('content') }}" rows="10"
