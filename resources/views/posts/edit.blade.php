        ...
        <form action="/posts/{{ $post->id }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="description"
                    value="{{ $post->description }}" placeholder="a small description for the post" />
                @error('description')
                    <p class="text-red-200 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="categories" class="border border-gray-200 rounded p-2 w-full">
            ...
            <div class="mb-6">
                ...
                <input class="border border-gray-200 rounded p-2 w-full" type="text" name="content"
                    value="{{ $post->content }}" />
                @error('content')
                    <p class="text-red-200 text-xs mt-1">{{ $message }}</p>
                @enderror
