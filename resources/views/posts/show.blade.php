            ...
            <div class="rounded">
                <div class="flex flex-col items-center justify-center text-center">
                    <img class="w-full md:block h-1/3"
                        src="{{ $post->cover ? asset('storage/' . $post->cover) : asset('/images/post.png') }}"
                        alt="" />
                    ...
                    <div class=" md:text-lg w-96 flex flex-col  text-start max-w-lg self-start">
                        {!!$post->content!!}
                    </div>
                </div>
            </div>
                ...
                @auth
                    @if (auth()->user()->id === $post->user_id)
                        <div class="actions flex gap-2 ">
                            ...
                            <button type="button" class="ms-2  btnComment font-bold hover:text-buttonBg"><i
                                    class="fa-solid fa-comment"></i> comment </button>
                            ...
                            <form method="post" action="/posts/{{ $post->id }}/comment">
                                <div class="comment_area hidden">
                                    ...
                                    <textarea rows="7" class="border mt-2 border-gray-200 rounded  p-2 w-full " name="comment"
                                        value="{{ old('comment') }}" placeholder="Enter The Comment Here"></textarea>
                                    <button type="submit"
                                        class=" bg-buttonBg px-4 mt-2 rounded-md text-white font-semibold py-2">
                                        Submit</button>
                                </div>
                            </form>
            ...
            <div class="comments flex gap-6 flex-col mt-4 w-96">
                @foreach ($comments as $comment)
                    <div class="flex flex-col gap-2 w-full  ">
                        <div class="border border-gray-400 bg-white px-3 py-4 rounded ">
